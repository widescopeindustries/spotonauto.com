...
async function main() {
  ...
  // ── 1. Use validated-vehicles.json as the canonical source ─────────────────
  // The JSON is now generated from CHARM + LEMON corpus data and normalized to
  // clean base model names, so it produces the right hub URLs.
  ...
  if (existsSync(VEHICLES_PATH)) {
    ...
    console.log(`  Canonical JSON: ${uniqueUrls.length} vehicle URLs`);
  }

  // ── 2. Optional DB sanity-check / fallback ────────────────────────────────
  ...
  if (uniqueUrls.length === 0) {
    console.error('No vehicle URLs could be generated. Check validated-vehicles.json and DB connection.');
    process.exit(1);
  }
