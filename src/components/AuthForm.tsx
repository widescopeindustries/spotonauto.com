  ...
  const handleResetPassword = async () => {
      ...
      try {
        await resetPassword(email.trim());
          setResetNotice('Your local profile is stored on this device. Signing in with the same email restores it.');
      } catch (err: any) {
          setError(err.message || 'Unable to update local profile.');
      }
  };
    ...
    <div className="w-full max-w-md mx-auto animate-fade-in">
        <div className="text-center mb-8">
            ...
            <p className="text-md text-gray-300 mt-1 font-body">
                {isLogin ? 'Sign in to restore your local repair history.' : 'Create a local profile to save guides on this device.'}
            </p>
        </div>
