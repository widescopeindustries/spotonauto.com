...
function ExcerptCard({ excerpt }: { excerpt: OEMExcerptData }) {
  ...
  const makeSlug = slugifyRoutePart(excerpt.make || '');
  const modelSlug = slugifyRoutePart(excerpt.model || '');
  const maintenanceHref = `/maintenance/${excerpt.year}/${makeSlug}/${modelSlug}`;
  ...
  return (
    <div className="rounded-xl border border-emerald-500/15 bg-emerald-950/20 p-4 md:p-5">
      ...
      <p className="text-sm leading-6 text-gray-400">
        Contains original factory repair procedures, specifications, and layout instructions utilized to build our guide.
      </p>
      ...
      <div className="mt-3 flex items-center gap-4">
        ...
        >
          View vehicle specifications hub
    ...
    <section className="mb-8 rounded-2xl border border-emerald-500/20 bg-emerald-500/[0.04] p-6 md:p-7">
      <div className="flex items-center gap-2.5 mb-1">
        ...
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-200/80">
          Grounded in Factory Specs
        </p>
      </div>
      <h3 className="text-xl md:text-2xl font-semibold tracking-tight text-white mt-2">
        Verified OEM references for {cleanTask}
      </h3>
      <p className="mt-2 text-sm leading-6 text-gray-400 max-w-2xl">
        To comply with copyright laws and protect manufacturer intellectual property, we do not reproduce raw factory manual pages. Instead, we have analyzed the following official source sections to synthesize our step-by-step instructions and specifications:
      </p>
